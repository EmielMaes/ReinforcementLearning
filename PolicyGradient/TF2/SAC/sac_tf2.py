...
class Agent:
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[8],
        ...
        # We use only 2 seperate learning rates, alpha for the actor and Beta for the value and critic network
        ...
        self.scale = reward_scale #this is the 1/alpha hyperparameter (below equation 1 in paper), so alpha here is actually 1/2
        self.update_network_parameters(tau=1)

    def choose_action(self, observation):
        state = tf.convert_to_tensor([observation])

        #Here, we don't really care about the logprobs at this stage
        actions, _ = self.actor.sample_normal(state, reparameterize=False)

        #Return actions[0] because the output is a tensor and we want a numpy array as output
        #the environment doesn't accept a tensor as input
        return actions[0]
    ...
    def update_network_parameters(self, tau=None):
        #This is used for updating the target value network cf hard update the first time
        # when tau = None and if we specify tau, then we do use a soft update
        if tau is None:
            tau = self.tau
    ...
    def learn(self):
        ...
        states_ = tf.convert_to_tensor(new_state, dtype=tf.float32) #next states
        ...
        with tf.GradientTape() as tape:
            ...
            # Get the value of the next state (will be used later when learning the critic network
            value_ = tf.squeeze(self.target_value(states_), 1)

            #We sample the actions from the current policy and use these to determine the action in the
            current_policy_actions, log_probs = self.actor.sample_normal(states,reparameterize=False)
            #Squeeze the logprobs
            log_probs = tf.squeeze(log_probs,1)
            #Get the Q values according to the 2 critic networks
            q1_new_policy = self.critic_1(states, current_policy_actions)
            q2_new_policy = self.critic_2(states, current_policy_actions)

            #The critic value is the min of the 2 Q values we calculated
        ...
        with tf.GradientTape() as tape:
            ...
            #Get the Q values from the 2 critic networks
            q1_new_policy = self.critic_1(states, new_policy_actions)
            q2_new_policy = self.critic_2(states, new_policy_actions)
            #Get the min Q value to be the critic value
            critic_value = tf.squeeze(tf.math.minimum(
                                        q1_new_policy, q2_new_policy), 1)
            #Calculate the loss as the log prob - critic value (see paper)
            actor_loss = log_probs - critic_value
            #Get the mean of all the losses
        ...
        #Learning the critic networks
        with tf.GradientTape(persistent=True) as tape:
            # Set the persisten to True because the loss will have 2 components (critic 1 and critic 2 loss)
            # we need to be able to keep track even after we applied gradients once because we need to apply them twice

            # I didn't know that these context managers shared values?
            # This value_ was calculated before when learning the value network
            # The scale variable reflects the 1/alpha used in the paper (below equation 1)
